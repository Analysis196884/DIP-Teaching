...
给定 3D 点 $P_i = [X_i, Y_i, Z_i]^T$ 和第 $j$ 个相机的变换矩阵 $R_j$ 与平移向量 $T_j$，点在相机坐标系下的坐标为：

$$
[X_{c}, Y_{c}, Z_{c}]^T = R_j P_i + T_j
$$
...
- **旋转矩阵**：使用欧拉角 $(\alpha, \beta, \gamma)$ 进行参数化，通过 `euler_angles_to_matrix` 转换为旋转矩阵 $R$。
...
$$
\min_{f, \{R_j, T_j\}, \{P_i\}} \sum_{i, j} w_{ij} \| \pi(P_i; f, R_j, T_j) - q_{ij} \|^2
$$

其中 $q_{ij}$ 为观测到的 2D 坐标，$w_{ij} \in \{0, 1\}$ 表示该点在对应视角下的可见性。
